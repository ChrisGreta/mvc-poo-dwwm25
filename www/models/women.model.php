<?php
    ...
    class Women {
        ...
        public function __construct($nom, $prenom, $date_naissance, $description, $image, $domaine = "informatique", $date_deces = null, $faits_historiques = []) {
            $this->nom = $nom;
            $this->prenom = $prenom;
        ...
        public function calculerAgeDeces() {
            if ($this->date_deces === "en vie" || $this->date_deces === null) {
                return "Toujours en vie";
            } else {
                $birthDate = new DateTime($this->date_naissance);
                $deathDate = new DateTime($this->date_deces);
                $age = $deathDate->diff($birthDate)->y;
                return $age . " ans";
            }
        }
    
       // This method generates an HTML unordered list (<ul>) of historical facts stored in the $faits_historiques array.
        // It loops through the array, appends each fact inside a <li> (list item) tag, and returns the complete HTML structure 
        // as a string. The final result is an unordered list (<ul>) with each fact listed as a bullet point (<li>).
        public function afficherFaitsHistoriques() {
            $output = "<ul>";  // Start the unordered list
            foreach ($this->faits_historiques as $fait) {  // Loop through each historical fact
                $output .= "<li>" . $fait . "</li>";  // Add each fact as a list item
            }
            $output .= "</ul>";  // Close the unordered list
            return $output;  // Return the HTML for the list of facts
        }


        public static function getWomen(){
            $json = file_get_contents("include/json/women.json");
            $data = json_decode($json, true);
            return $data["femmes_celebres"];
        }

    
    //     public function afficherTableauFaitsHistoriques() {
    //         $output = "<table style='width:100%; border-collapse: collapse;'>";
    //         $output .= "<tr><th style='border: 1px solid #ddd; padding: 8px;'>Fait Historique</th></tr>";
    //         foreach ($this->faits_historiques as $fait) {
    //             $output .= "<tr><td style='border: 1px solid #ddd; padding: 8px;'>" . $fait . "</td></tr>";
    //         }
    //         $output .= "</table>";
    //         return $output;
    //     }
